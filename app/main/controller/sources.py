...
def create_source():
    """Create a source of ingestion.
    post:
    summary: Create a source of ingestion service.
    requestBody:
        description: DataSource object that describes the source to be created.
        required: true
    """
    rest_manager = RestManager()
    try:
        data = request.json
        source = get_request_json_as_dataclass(DataSource, data)
        airbyte_ingeter = AirbyteIngestor()
        data = airbyte_ingeter.add_source(source)
...
def delete_source():
    ...
    try:
        ...
        source = get_request_json_as_dataclass(DataSource, data)
        airbyte_ingeter = AirbyteIngestor()
        data = airbyte_ingeter.remove_source(source)
        return rest_manager.rest_response(
...
def view_source():
    """View the source of ingestion.
    post:
    summary: View a source of ingestion service.
    requestBody:
        description: DataSource object that describes the source to be viewed.
        required: true
    """
    rest_manager = RestManager()
    try:
        ...
        source = get_request_json_as_dataclass(DataSource, data)
        airbyte_ingeter = AirbyteIngestor()
        data = airbyte_ingeter.get_source(source)
        return rest_manager.rest_response(
            data=asdict(data), status=constants.SUCCESS, status_code=200
        )
    except (TypeError, KeyError, AirbyteError):
...
def update_source():
    """Update a source of ingestion.
    post:
    summary: Create a source of ingestion service.
    requestBody:
        description: DataSource object that describes the source to be updated.
        required: true
    """
    rest_manager = RestManager()
    try:
        ...
        source = get_request_json_as_dataclass(DataSource, data)
        airbyte_ingeter = AirbyteIngestor()
        data = airbyte_ingeter.update_source(source)
        return rest_manager.rest_response(
            data=asdict(data), status=constants.SUCCESS, status_code=200
        )
    except (TypeError, KeyError, AirbyteError):
